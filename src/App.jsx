...
const maskCnpj = (v) => v ? v.replace(/\D/g, '').replace(/(\d{2})(\d)/, '$1.$2').replace(/(\d{3})(\d)/, '$1.$2').replace(/(\d{3})(\d)/, '$1/$2').replace(/(\d{4})(\d)/, '$1-$2').replace(/(-\d{2})\d+?$/, '$1') : "";
...
const MobileView = ({ user, initialRole, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 pb-32 font-sans max-w-md mx-auto shadow-2xl relative">
      ...
      {view==='menu' && (
          <div>
              <div className="bg-white p-3 sticky top-[60px] z-20 border-b shadow-sm"><div className="relative mb-3"><Search className="absolute left-3 top-2.5 text-slate-400" size={18}/><input value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} placeholder="Buscar..." className="w-full bg-slate-100 pl-10 p-2 rounded-lg text-sm outline-none"/></div><div className="flex gap-2 overflow-x-auto pb-1">{categories.map(c=>(<button key={c} onClick={()=>setSelectedCategory(c)} className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap ${selectedCategory===c?'bg-slate-900 text-white':'bg-slate-100'}`}>{c}</button>))}</div></div>
...
const PosView = ({ user, onBack, initialSettings, refreshSettings }) => {
  ...
  const [newProdCost, setNewProdCost] = useState(''); // Custo
  ...
  return (
    <div className="font-sans bg-slate-100 min-h-screen text-slate-900 flex">
      ...
      <div className="pl-16 w-full">
        ...
        {view === 'admin' && (
            <div className="p-8 h-screen overflow-y-auto">
                ...
                <div className="flex gap-2 mb-4"><button onClick={()=>setShowCashMovementModal(true)} className="bg-indigo-600 text-white px-4 py-2 rounded flex items-center gap-2"><ArrowRightLeft size={16}/> Movimentação</button></div>
      ...
      {showCashMovementModal && (<div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center"><div className="bg-white p-6 rounded-xl"><h3 className="font-bold mb-4">Nova Movimentação</h3><div className="flex gap-2 mb-4"><button onClick={()=>setMovementType('suprimento')} className="flex-1 bg-green-100 text-green-700 font-bold p-2 rounded">Entrada</button><button onClick={()=>setMovementType('sangria')} className="flex-1 bg-red-100 text-red-700 font-bold p-2 rounded">Saída</button></div><input type="number" value={movementValue} onChange={e=>setMovementValue(e.target.value)} placeholder="Valor" className="border p-2 w-full mb-2"/><input value={movementDesc} onChange={e=>setMovementDesc(e.target.value)} placeholder="Descrição" className="border p-2 w-full mb-4"/><button onClick={()=>{/*Lógica simplificada*/alert('Salvo!');setShowCashMovementModal(false);}} className="bg-indigo-600 text-white w-full py-2 rounded font-bold">Salvar</button></div></div>)}
