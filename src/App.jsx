...
const version = "2.0.0"; // Versão com Montagem de Pratos
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        if (e.code === 'auth/operation-not-allowed') {
          setErrorMsg("Erro: Ative o login Anônimo no Console do Firebase.");
        }
      }
    };
    initAuth();
    ...
    const unsubscribe = onAuthStateChanged(auth, u => {
      setFirebaseUser(u);
      if (u) {
        getDoc(getDocRef('app_state', 'settings')).then(s => {
  ...
  const handleClientLogin = () => {
    if (!clientNameInput || !clientPhoneInput) {
      showToastMsg("Por favor, preencha seu nome e telefone.", "error");
      return;
    }
  ...
  if (errorMsg) return <div className="h-screen flex items-center justify-center bg-slate-900 p-6 text-center"><div className="bg-red-500/10 border border-red-500 text-red-500 p-6 rounded-2xl max-w-md"><AlertTriangle size={48} className="mx-auto mb-4" /><p className="font-bold">{errorMsg}</p></div></div>;
  if (!firebaseUser) return <div className="h-screen flex items-center justify-center bg-slate-900 text-white"><div className="flex flex-col items-center gap-4"><Loader2 size={48} className="animate-spin text-blue-500" /><p className="text-slate-400 font-medium tracking-widest uppercase text-sm">Iniciando Sistema...</p></div></div>;
       ...
       appMode === 'pos' ? <PosView user={firebaseUser} onBack={() => setAppMode('landing')} initialSettings={storeSettings} refreshSettings={setStoreSettings} /> :
        <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
          ...
          <div className="z-10 w-full max-w-5xl text-center">
            <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-white/5 border border-white/10 mb-8 shadow-2xl backdrop-blur-sm">
               <Store size={40} className="text-blue-400" />
            </div>
            <h1 className="text-4xl md:text-6xl font-black text-white mb-4 tracking-tight drop-shadow-lg">{storeSettings.storeName || "CONFEITARIA & CAFÉ"}</h1>
            <h2 className="text-xl md:text-2xl text-blue-300 font-medium mb-16 tracking-wide">Plataforma de Gestão Integrada</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
              <button onClick={() => setShowClientModal(true)} className="group bg-white/5 hover:bg-white/10 border border-white/10 hover:border-blue-500/50 p-8 rounded-3xl transition-all duration-300 hover:-translate-y-2 backdrop-blur-sm relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <div className="w-20 h-20 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl mx-auto mb-6 flex items-center justify-center shadow-lg shadow-blue-500/30 group-hover:scale-110 transition-transform"><UserCircle2 size={40} className="text-white" /></div>
                <h2 className="text-2xl font-bold text-white mb-2 relative z-10">Cliente</h2>
                <p className="text-sm text-slate-400 font-medium relative z-10">Acesso ao Cardápio Digital.</p>
              </button>
              
              <button onClick={() => { setMobileRole('Garçom'); setAppMode('mobile'); }} className="group bg-white/5 hover:bg-white/10 border border-white/10 hover:border-orange-500/50 p-8 rounded-3xl transition-all duration-300 hover:-translate-y-2 backdrop-blur-sm relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-orange-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <div className="w-20 h-20 bg-gradient-to-br from-orange-400 to-orange-600 rounded-2xl mx-auto mb-6 flex items-center justify-center shadow-lg shadow-orange-500/30 group-hover:scale-110 transition-transform"><ChefHat size={40} className="text-white" /></div>
                <h2 className="text-2xl font-bold text-white mb-2 relative z-10">Garçom</h2>
                <p className="text-sm text-slate-400 font-medium relative z-10">Lançamento de Pedidos.</p>
              </button>
              
              <button onClick={() => setShowPasswordModal(true)} className="group bg-white/5 hover:bg-white/10 border border-white/10 hover:border-purple-500/50 p-8 rounded-3xl transition-all duration-300 hover:-translate-y-2 backdrop-blur-sm relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-purple-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <div className="w-20 h-20 bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl mx-auto mb-6 flex items-center justify-center shadow-lg shadow-purple-500/30 group-hover:scale-110 transition-transform"><MonitorSmartphone size={40} className="text-white" /></div>
                <h2 className="text-2xl font-bold text-white mb-2 relative z-10">Caixa / Gestão</h2>
                <p className="text-sm text-slate-400 font-medium relative z-10">Controle completo da loja.</p>
              </button>
            </div>
          </div>
          ...
          {showPasswordModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
              <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl animate-in zoom-in-95 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-purple-500 to-pink-600"></div>
                <div className="flex justify-center mb-6"><div className="w-16 h-16 bg-purple-50 text-purple-600 rounded-2xl flex items-center justify-center"><Lock size={32} /></div></div>
                <h3 className="text-2xl font-black text-slate-800 mb-2 text-center">Acesso Restrito</h3>
                <p className="text-sm text-slate-500 mb-6 text-center font-medium">Informe a senha gerencial</p>
                <input type="password" autoFocus placeholder="••••" className="w-full border-2 border-slate-200 rounded-2xl p-4 text-center text-3xl font-black tracking-widest outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-100 transition-all mb-6 text-slate-800" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submitPassword()} />
                <div className="flex gap-3">
                  <button onClick={() => { setShowPasswordModal(false); setPasswordInput(''); }} className="flex-1 py-4 text-slate-600 font-bold bg-slate-100 hover:bg-slate-200 rounded-2xl transition-colors">Cancelar</button>
                  <button onClick={submitPassword} className="flex-1 bg-purple-600 text-white py-4 rounded-2xl font-black hover:bg-purple-700 transition-all shadow-lg shadow-purple-500/30 active:scale-95">Entrar</button>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
              <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl animate-in zoom-in-95 relative overflow-hidden">
                ...
                <div className="text-center mb-6">
                  <div className="w-20 h-20 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-white shadow-sm"><UserCircle2 size={40} /></div>
                  <h3 className="text-2xl font-black text-slate-800 mb-1">Bem-vindo(a)!</h3>
                  <p className="text-sm text-slate-500 font-medium">Identifique-se para começar.</p>
                </div>
                <div className="space-y-4 mb-8">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Como devemos te chamar?</label>
                    <input autoFocus placeholder="Seu Nome" className="w-full border-2 border-slate-200 rounded-2xl p-4 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all font-bold text-slate-800" value={clientNameInput} onChange={(e) => setClientNameInput(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2 ml-1">Seu WhatsApp</label>
                    <input placeholder="(00) 00000-0000" className="w-full border-2 border-slate-200 rounded-2xl p-4 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all font-bold text-slate-800" value={clientPhoneInput} onChange={(e) => setClientPhoneInput(e.target.value)} />
                  </div>
                </div>
                <div className="flex flex-col gap-3">
                  <button onClick={handleClientLogin} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/30 active:scale-95 text-lg">Acessar Cardápio</button>
                  <button onClick={() => setShowClientModal(false)} className="w-full py-4 text-slate-500 font-bold hover:bg-slate-100 rounded-2xl transition-colors">Voltar</button>
                </div>
              </div>
            </div>
          )}
          ...
          <footer className="absolute bottom-6 w-full text-center z-10">
            <p className="text-sm text-slate-500 font-medium opacity-80 flex items-center justify-center gap-2">
              <Store size={14}/> Plataforma de Gestão v{version}
            </p>
          </footer>
        </div>
      }
